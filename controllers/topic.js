...
exports.getGeneratedRecords = getGeneratedRecords;
...
function generate(req,res,next){
	...
	var resData = {};
	resData.title = "Generate";
	resData.email = req.session.user.userEmail;

	Users.getByEmail(req.session.user.userEmail,function(error,user){
		if (error) {
	      	return next(error);
	    }
	    if (user) {
				res.render('topic/generate', {title: "Generate", user: user});
	    }else{
	    	res.render('topic/generate', {title: "Generate", results: []});
	    }
	});
	
}

function getGeneratedRecords(req,res,next){
	
	console.log(req.body);
	var finalResult = [];
	program_path = '/home/murtaza/NodeJS-forum-master/python_gen/';
	var PythonShell = require('python-shell');
	var options = {
	mode: 'text',
	pythonPath: '/home/murtaza/anaconda2/bin/python',
	pythonOptions: ['-u'],
	scriptPath: program_path,
	//args: ['1', '5']
	//args: [program_path, '1', '5']
	args: [program_path, req.body.interest, req.body.skills]
	};
	PythonShell.run('test.py', options, function (err, results) {
	if (err) throw err;
	// results is an array consisting of messages collected during execution
	console.log('results: %j', results);
	finalResult = results[0].split("||");
...
function add(req,res,next){
	...
	var topic_title = req.query.topic_title;
	var tags = req.query.tags;


	if(topic_title != undefined && tags != undefined){
		var resData = {
			title : "Add"
			
		};
	
		resData.topic_title = topic_title;
		resData.tagsName = tags;
		res.render('topic/create', resData);
		return;
	}
	console.log(req.query.topic_title);
