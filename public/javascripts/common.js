...
function init(){
	...
	var createForum = $("#form-create");
	if(createForum.length > 0 ){
		createForum.submit(function(e) {
			debugger
			var a = "";
		});
	}
	...
	var getUserInputs = $("#getUserInputs");
	if(getUserInputs.length > 0 ){
		getUserInputs.submit(function(e) {
			ajaxPost(e,getUserInputs,"/topic/generate");
		});
	}

	var getGeneratedRecords = $("#generateForm");
	if(getGeneratedRecords.length > 0 ){
		getGeneratedRecords.submit(function(e) {
			var interest = $("#example-getting-started").val();
			var skills = $(".example-getting-started").val();
				$.ajax({
					type: "POST",
					url: "/topic/getGeneratedRecords",
					timeout : 10000,
					dataType: "json",
					data: { 'interest': interest, 'skills': skills },
					success: function(data){
							var results = data.results;
							$('#generate-table tbody')[0].innerHTML = "";
							
							tags = $("#example-getting-started").val() + "," + $(".example-getting-started").val();
							results.forEach(function(result){
								if(result != ""){
									$('#generate-table').append('<tr>' +
									'<td>'+ result+'</td>' +
				
									'<td><form action="/topic/add" method="GET"><button type = "submit" data-val="' + result + '" class="btn btn-primary" >Create Forum</button><input type="hidden" id="postId" name="topic_title" value="' + result +'"> <input type="hidden" id="postId1" name="tags" value="' + tags +'"></form></td>' +
													'</tr> ');
								}
